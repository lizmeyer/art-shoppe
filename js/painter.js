...
// Add this to painter.js after the existing code

// Art adjustment controls
function addArtPositionControls() {
    // Don't add if we already have controls or don't have a product selected
    if (document.getElementById('artAdjustmentControls') || !painter.selectedProduct) return;
    ...
    controlsContainer.id = 'artAdjustmentControls';
    controlsContainer.style.margin = '15px 0';
    controlsContainer.style.padding = '10px';
    controlsContainer.style.backgroundColor = 'var(--bg-secondary)';
    controlsContainer.style.borderRadius = 'var(--border-radius)';
    
    // Create title
    const title = document.createElement('div');
    title.textContent = 'Adjust Artwork Placement';
    ...
    const currentPos = painter.selectedProduct.artPosition || getArtPosition(painter.selectedProduct.id);
    
    // Parse percentage values to numbers for easier manipulation
    const top = parseInt(currentPos.top) || 25;
    const left = parseInt(currentPos.left) || 25;
    const width = parseInt(currentPos.width) || 50;
    const height = parseInt(currentPos.height) || 50;
    
    // Create sliders for each property
    const properties = [
        { name: 'top', label: 'Top', value: top, min: 0, max: 80 },
        { name: 'left', label: 'Left', value: left, min: 0, max: 80 },
        { name: 'width', label: 'Width', value: width, min: 20, max: 100 },
        { name: 'height', label: 'Height', value: height, min: 20, max: 100 }
    ];
    ...
    properties.forEach(prop => {
        ...
        slider.addEventListener('input', () => {
            ...
            // Update product art position
            if (!painter.selectedProduct.artPosition) {
                painter.selectedProduct.artPosition = { ...currentPos };
            }
            
            painter.selectedProduct.artPosition[prop.name] = `${slider.value}%`;
        ...
        positionControls.appendChild(control);
    });
    ...
    resetButton.addEventListener('click', () => {
        ...
        const defaultPosition = getArtPosition(painter.selectedProduct.id);
        painter.selectedProduct.artPosition = { ...defaultPosition };
        
        // Update slider values
        const sliders = controlsContainer.querySelectorAll('input[type="range"]');
        const valueDisplays = controlsContainer.querySelectorAll('span');
        
        sliders.forEach((slider, index) => {
            const prop = properties[index].name;
            const value = parseInt(defaultPosition[prop]);
            slider.value = value;
    ...
    // Add to product preview area
    const previewParent = previewEl.parentElement;
    previewParent.appendChild(controlsContainer);
}

// Update the selectProduct function to add controls when a product is selected
const originalSelectProduct = selectProduct;
selectProduct = function(productId) {
    originalSelectProduct(productId);
    
    // Add adjustment controls after product is selected
    setTimeout(addArtPositionControls, 100);
};

// Modify updateProductPreview to use custom art positions if available
const originalUpdateProductPreview = updateProductPreview;
updateProductPreview = function() {
    if (painter.selectedProduct && painter.selectedProduct.artPosition) {
        // Use custom position if set
        originalUpdateProductPreview();
    } else {
    ...
    if (!document.getElementById('artAdjustmentControls')) {
        setTimeout(addArtPositionControls, 100);
    }
};

// Update createProduct to save custom positions with the product
const originalCreateProduct = createProduct;
createProduct = function() {
    // Store the custom position if it exists
    if (painter.selectedProduct && painter.selectedProduct.artPosition) {
        // Make sure we capture the custom position in the product
        const customPosition = { ...painter.selectedProduct.artPosition };
        
        // Override the original function
        const original = addToInventory;
        addToInventory = function(product) {
            // Add the custom position
            product.customArtPosition = customPosition;
            return original(product);
        };
        
        // Call the original function
        originalCreateProduct();
        
        // Restore the original function
        addToInventory = original;
    } else {
...
// Update showCreatedProduct to use custom positions
const originalShowCreatedProduct = showCreatedProduct;
showCreatedProduct = function(product) {
    // If the product has a custom position, use it
    if (product.customArtPosition) {
        // Save original position
        const originalArtPosition = getArtPosition;
        
        // Override the getArtPosition function temporarily
        getArtPosition = function() {
            return product.customArtPosition;
        };
        
        // Call the original function
        originalShowCreatedProduct(product);
        
        // Restore the original function
        getArtPosition = originalArtPosition;
    } else {
        // Just call the original function
        originalShowCreatedProduct(product);
    }
